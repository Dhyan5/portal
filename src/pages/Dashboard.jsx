...
import { fetchDrives, addDrive, editDrive, deleteDrive, toggleDriveStatus } from '../api/sheets';
...
export default function Dashboard() {
  ...
  const handleToggleStatus = async (id) => {
    ...
    if (result.success) {
      setDrives(drives.map(d => d.id === id ? { ...d, status: result.data.status } : d));
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] bg-surface-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold font-display text-surface-900">Dashboard</h1>
                <p className="text-sm text-surface-500">Manage placement drives</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-3 gap-3 mt-5">
            ...
            <div className="p-3 rounded-xl bg-emerald-50 border border-emerald-100">
              ...
              <p className="text-xs text-emerald-600 mt-0.5">Open</p>
            </div>
            <div className="p-3 rounded-xl bg-red-50 border border-red-100">
              <p className="text-2xl font-bold text-red-700">{closedCount}</p>
              <p className="text-xs text-red-600 mt-0.5">Closed</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {loading ? (
          <LoadingSpinner text="Loading drives..." />
          ...
          <div className="text-center py-16 animate-fade-in">
            ...
            <h3 className="text-lg font-semibold text-surface-800 mb-2">
              {drives.length === 0 ? 'No drives yet' : 'No results'}
            </h3>
            <p className="text-sm text-surface-500 mb-4">
              {drives.length === 0 ? 'Add your first placement drive to get started.' : 'Try a different search term.'}
            </p>
            {drives.length === 0 && (
          ...
          <div className="space-y-3">
            {filteredDrives.map((drive) => (
              <div
                key={drive.id}
                className="bg-white rounded-xl border border-surface-100 p-4 sm:p-5 hover:shadow-md transition-shadow animate-fade-in"
              >
                <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                  ...
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={drive.status === 'Open' ? 'badge-open' : 'badge-closed'}>
                          <span className={`w-1.5 h-1.5 rounded-full ${drive.status === 'Open' ? 'bg-emerald-500' : 'bg-red-500'}`} />
                          {drive.status}
                        </span>
                      </div>
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1 text-xs text-surface-500">
                        ...
                        <span className="flex items-center gap-1"><IndianRupee className="w-3 h-3" />{drive.package || '—'}</span>
                        <span className="flex items-center gap-1"><Calendar className="w-3 h-3" />{formatDate(drive.drive_date)}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0 sm:ml-auto">
                    <button
                      ...
                      title={`Toggle to ${drive.status === 'Open' ? 'Closed' : 'Open'}`}
                    >
                      {drive.status === 'Open' ? <ToggleRight className="w-5 h-5" /> : <ToggleLeft className="w-5 h-5" />}
                    </button>
                    ...
                    <button
                      onClick={() => {
                        if (window.confirm(`Delete "${drive.name}" drive? This cannot be undone.`)) {
